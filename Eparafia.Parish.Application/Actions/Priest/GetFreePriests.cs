...
public static class GetFreePriests
{
    ...
    public class Handler : IRequestHandler<Query, List<PublicPriestProfile>>
    {
        ...
        public async Task<List<PublicPriestProfile>?> Handle(Query request, CancellationToken cancellationToken)
        {
            ...
            var priests =
                await _unitOfWork.Priests.GetFreePriestAsync(query, request.Page, _pageSize, cancellationToken);

            if (priests is null) throw new InvalidRequestException("Not found result");

            var result = new List<PublicPriestProfile>();
            foreach (var priest in priests) result.Add(PublicPriestProfile.Create(priest));
