...
public static class GetCalendar
{
    ...
    public class Handler : IRequestHandler<Query, List<SpecialEvent>>
    {
        ...
        public async Task<List<SpecialEvent>> Handle(Query request, CancellationToken cancellationToken)
        {
            ...
            var startOfWeek =
                DateTime.Today.AddDays(-(int)DateTime.Today.DayOfWeek + (int)DayOfWeek.Monday + request.Week * 7);
            var specialEvents =
                await _unitOfWork.SpecialEvents.GetForWeek(request.ParishId, startOfWeek, cancellationToken);
            ...
            foreach (var specialEvent in specialEvents) calendar.Add(specialEvent);
