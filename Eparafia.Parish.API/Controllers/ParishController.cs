...
public class ParishController : Controller
{
    ...
    [HttpPost]
    public async Task<IActionResult> CreateParish(CreateParish.Command command,
    ...
    [HttpDelete]
    public async Task<IActionResult> RemoveParish(RemoveParish.Command command,
        CancellationToken cancellationToken = default)
    {
        await _mediator.Send(command, cancellationToken);
        return Ok(ApiResponse.Success(200));
    ...
    [HttpPut]
    public async Task<IActionResult> UpdateParish(UpdateParish.Command command,
        CancellationToken cancellationToken = default)
    {
        await _mediator.Send(command, cancellationToken);
        return Ok(ApiResponse.Success(200));
    ...
    public async Task<IActionResult> GetParishByShortName(string shortName,
        CancellationToken cancellationToken = default)
    {
