...
{
    public sealed record Command(Guid PostId, string? Title, string? Content, List<Base64File>? Files, List<Guid>? FilesToRemove) : IRequest<Unit>;
    ...
    public class Handler : IRequestHandler<Command, Unit>
    {
        ...
        public async Task<Unit> Handle(Command request, CancellationToken cancellationToken)
        {
            ...
            if (request.Files is not null)
            {
                ...
                {
                    var filePaths = await _fileManager.SaveImageAsync(file.Base64, ImageType.PostPhoto, Guid.NewGuid(), cancellationToken);
