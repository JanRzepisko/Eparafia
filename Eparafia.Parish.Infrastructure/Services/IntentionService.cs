...
{
    public async Task<DateTime> CalculateNextIntentionDateAsync(Guid parishId, string content, IUnitOfWork unitOfWork,
        CancellationToken cancellationToken, int startWeek = 0)
    {
        ...
        for (var i = 1;; i++)
        {
            ...
            foreach (var @event in week)
            {
                ...
                {
                    var date = startOfWeek.AddDays((int)@event.DayOfWeek).AddHours(@event.Time.Hours)
